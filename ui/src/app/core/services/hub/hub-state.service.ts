...
export class HubStateService {
    ...
    storeFeatured = computed(() => this.enrichedApps().slice(0, 3).map(app => ({ id: app.id, title: app.name, description: app.description || "Découvrez la puissance de l'écosystème EtherNanos.", image: app.banner_url || 'https://images.unsplash.com/photo-1614850523296-d8c1af93d400?q=70&w=1200&auto=format&fit=crop', action: 'Déverrouiller', price: app.hasDiscount ? app.price : 'Inclus' })));
